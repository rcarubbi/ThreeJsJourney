...
gui.add(parameteres, 'speed').min(0).max(100).step(.01)
gui.add(parameteres, 'amplitude').min(0).max(10).step(.01)
...
fontLoader.load(
    ...
    (font) => {
        ...
        gui.add(parameteres, 'donutCounter').min(0).max(1000).step(1).onChange(() => {
...
const tick = () => {
    ...
    camera.position.x = Math.cos(elapsedTime * parameteres.speed) * parameteres.amplitude
    camera.position.y = Math.sin(elapsedTime * parameteres.speed) * parameteres.amplitude
